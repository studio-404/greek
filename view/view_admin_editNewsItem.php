				...
				<form action="" method="post" class="my-form hundredPorsent" autocomplete="off" enctype="multipart/form-data">
					<?php
						if(isset($_GET['type']) && $_GET['type']=="eventpage"){
							echo '<div class="from-header" style="color:#ef4836; text-transform:uppercase">Edit event item</div>';
						}else{
							echo '<div class="from-header" style="color:#ef4836; text-transform:uppercase">Edit news item</div>'; 
						}
					?>
					...
					<input type="submit" name="edit_news_item" id="submit" value="Submit"><br>
					<button id="cancel" onclick="redirect('_self','<?=WEBSITE.LANG?>/admin?action=newsModule&type=newspage&id=<?=$_GET['id']?>&super=<?=$_GET['super']?>&token=<?=$_SESSION['token']?>'); return false;">Back</button>
				</form>
			</div>
		</div>
	</main>
