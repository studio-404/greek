...
function openPromt(maxidx){
    ...
    if (newFileNmae != null) {
    	$.get(root+"/en/ajaxupload",{ id:fileid[1], filename:newFileNmae },function(data){
...
function openPromt2(maxidx){
    ...
    if (newFileNmae != null) {
    	$.get(root+"/en/ajaxupload",{ id:fileid[1], filename:newFileNmae, photoedittitle:"true" },function(data){
...
function removeFile(maxidx){
	...
	$.get(root+"/en/ajaxupload",{ idx:maxidx, idxes2:send_idx_array }, function(data){
		console.log(data);
	});
}
...
function removeFile2(maxidx){
	...
	if(send_idx_array==""){ send_idx_array=maxidx; }
	$.get(root+"/en/ajaxupload",{ idx:maxidx, idxes3:send_idx_array }, function(data){
		console.log(data);
	});
}
...
function removeFile3(maxidx){
	...
	$.get(root+"/en/ajaxupload",{ idx:maxidx, idxes3:send_idx_array, media_type:"video" }, function(data){
		console.log(data);
	});
}
...
function getDate(x)
{
	...
	if(!executed){
		...
		xhr.open('post',root+'/en/session_timeout',true);
		xhr.send();
