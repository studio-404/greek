...
class edit_page_interface extends connection{
	...
	public function out_interface($c){
		...
		switch($page_type){
			...
			if(isset($_GET['action']) && $_GET['action']=="editNewsItem"){
				...
				`studio404_module_item`.`event_booth` AS smi_event_booth,  
	...
	public function general_form_news($fetch){
		...
		if($fetch["sma_page_type"]=="eventpage"){ // if eventpage
			//event_booth
			$out .= '<label for="event_booth">Booth №: </label>';
			$out .= '<input type="text" name="event_booth" id="event_booth" value="'.htmlentities($fetch["smi_event_booth"]).'" autocomplete="off">';			
			...
			$out .= '<label for="event_fee">Event place: </label>';
